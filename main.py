...
def main(args):
    ...
    for i in folds:
        ...
        results, test_auc, val_auc, test_acc, val_acc, f1, sensitivity, specificity  = train(datasets, i, args)
    ...
    final_df = pd.DataFrame({'folds': folds, 'test_auc': all_test_auc, 
        'val_auc': all_val_auc, 'test_acc': all_test_acc, 'val_acc' : all_val_acc, 'test_f1' : all_f1, 'test_sen' : all_sen, 'test_spe' : all_spe})
    ...
    mean_f1_test = final_df['test_f1'].mean()
    std_f1_test = final_df['test_f1'].std()
    mean_sen_test = final_df['test_sen'].mean()
    std_sen_test = final_df['test_sen'].std()
    mean_spe_test = final_df['test_spe'].mean()
    std_spe_test = final_df['test_spe'].std()
    ...
    df_append = pd.DataFrame({
        ...
        'test_f1': [mean_f1_test, std_f1_test],
        'test_sen': [mean_sen_test, std_sen_test],
        'test_spe': [mean_spe_test, std_spe_test],
    })
...
if args.task == 'LUAD_LUSC512':
    ...
    args.n_classes=2
    dataset = Generic_MIL_Dataset(csv_path = '/data/MambaMIL/dataset_csv/C16512_new.csv',
                            data_dir= None,
                            shuffle = False, 
                            seed = args.seed, 
                            print_info = True,
